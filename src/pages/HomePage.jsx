  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-8 lg:px-12 mt-6 sm:mt-8 flex flex-col gap-12 sm:gap-16 w-full">
      ...
      <section>
        ...
        {/* Mobile: scroll row  |  Desktop: evenly spaced */}
      ...
      <section className="pt-4 sm:pt-8">
        <div className="flex justify-between items-end mb-6 sm:mb-8">
          <div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-[#0F172A] tracking-tight mb-1 sm:mb-2">The Weekly Edit</h2>
            <p className="text-sm text-gray-500 font-medium hidden sm:block">Handpicked essentials for the modern lifestyle.</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:h-[600px]">
          {/* Left Big Card */}
          <div className="relative rounded-3xl overflow-hidden bg-gray-100 group cursor-pointer border border-gray-100 shadow-sm
                          h-[300px] sm:h-[400px] lg:h-full">
            <img
              src={WEEKLY_EDIT_MAIN.image}
              className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
              alt={WEEKLY_EDIT_MAIN.name}
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#0F172A]/90 via-[#0F172A]/10 to-[#0F172A]/20 duration-500 group-hover:from-black" />

            <div className="absolute top-5 left-5 sm:top-6 sm:left-6 grid gap-2">
            ...
            <div className="absolute bottom-6 left-6 sm:bottom-8 sm:left-8 text-white">
              <h3 className="text-2xl sm:text-4xl font-extrabold tracking-tight mb-1 sm:mb-2">{WEEKLY_EDIT_MAIN.name}</h3>
              <p className="text-gray-300 font-medium uppercase tracking-wider text-[10px] drop-shadow-md">{WEEKLY_EDIT_MAIN.category}</p>
            </div>
          </div>

          {/* Right Quad Grid */}
          <div className="grid grid-cols-2 gap-4 sm:gap-6 lg:h-full">
      ...
      <section className="bg-[#EEF2FF] rounded-[24px] sm:rounded-[32px] border border-[#E0E7FF] mt-4 sm:mt-8
        ...
        <div className="max-w-xl">
          <h2 className="text-2xl sm:text-[32px] font-extrabold text-[#0F172A] mb-3 tracking-tight">Artisan Curation</h2>
          <p className="text-[#475569] text-sm leading-relaxed font-medium">
            Every item in Shopter is vetted for design integrity and lasting quality. Join our membership for exclusive early access to heritage drops.
          </p>
        </div>
      ...
      <section className="pt-4 sm:pt-8 pb-8">
        <div className="flex justify-between items-end mb-6 sm:mb-8 border-b border-gray-200 pb-4">
          <div>
            <h2 className="text-xl sm:text-2xl font-extrabold text-[#0F172A] tracking-tight mb-1 sm:mb-2">Suggested For You</h2>
            <p className="text-sm text-gray-500 font-medium hidden sm:block">Picked based on trending collections.</p>
          </div>
          <div className="flex gap-2">
            <button onClick={scrollLeft} className="p-2 rounded-full border border-gray-200 text-gray-500 hover:bg-gray-100 transition-colors hover:text-gray-900">
              <ChevronLeft size={16} />
            </button>
            <button onClick={scrollRight} className="p-2 rounded-full border border-gray-200 text-gray-500 hover:bg-gray-100 transition-colors hover:text-gray-900">
              <ChevronRight size={16} />
            </button>
          </div>
        </div>

        <div ref={scrollRef} className="flex overflow-x-auto gap-4 sm:gap-6 pb-6 pt-2 scrollbar-hide snap-x px-1">
          {SUGGESTED_PRODUCTS.map((item, idx) => (
            <div key={idx} className="snap-start min-w-[220px] sm:min-w-[280px] shrink-0">
